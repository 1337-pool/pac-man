...
class MazeGenerationError(Exception):
    """Raised when the external maze generator fails or receives invalid parameters."""
    pass
...
def generate_maze(width: int, height: int, seed: int) -> MazeGenerator:
    ...
    try:
        maze: MazeGenerator = MazeGenerator(size=(width, height), seed=seed, perfect=False)
...
def decode_cell(value: int) -> dict[str, bool]:
    ...
    result: dict[str, bool] = {"north": False, "east": False, "south": False, "west": False}
...
def decode_maze(raw_maze: list[list[int]]) -> list[list[dict[str, bool]]]:
    """Convert a 2D array of bitmask integers into a 2D array of wall dictionaries."""
